  ...
  ReentrancyGuard
{
  ...
  function createLease(
    ...
    if (leaseCondition.paymentTokenAddress == address(0)) {
      ...
      paymentTokenAllowlist[leaseCondition.paymentTokenAddress] == ERC20_TOKEN
    ) {
    ...
    onlyAllowlisted
  {
    ...
    if (fees2Redeem > 0) {
      lease2redeem.feesToPullRemaining = lease2redeem.feesToPullRemaining.sub(
        fees2Redeem
      );
    }
    ...
    if (lease2redeem.paymentTokenAddress == address(0)) {
      ...
      paymentTokenAllowlist[lease2redeem.paymentTokenAddress] == ERC20_TOKEN
    ) {
  ...
  function onERC721Received(
    ...
    require(!allowlistEnabled || _isAllowlisted(operator), "User not allowed");
